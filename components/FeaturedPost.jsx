...
const FeaturedPost = async () => {
    ...
    return (
        <div id='featuredBody' className=' relative lg:h-96 w-full'>
            <Image src={`${postImage}`} height={800} width={600} className='absolute z-0 w-full h-full object-cover brightness-50' />
            ...
            <section className='relative p-5 md:p-10 flex flex-col justify-center h-full z-10 text-white lg:w-2/3 gap-5'>
                <p className='intern-text-p1'>POSTED ON <a href={`/categories/${featuredPost[0].categories[0].slug}`} className='text-yellow-300 uppercase'>{featuredPost[0].categories[0].name}</a> | {moment(featuredPost[0].createdAt).format('MMM DD, YYYY')}</p>
                <h1 className='text-yellow-500'><a href={`/posts/${featuredPost[0].slug}`}>{featuredPost[0].title}</a></h1>
